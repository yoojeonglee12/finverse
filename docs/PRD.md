...
| | `institution` | 지표를 움직이는 기관·정부 | 미 연준, 한국은행, ECB, 일본은행, 기재부, 미 재무부, 금융위, IMF | ✅ 지표와 연결 |
| | `event` | 예정·발생한 경제 이벤트 | FOMC, 금통위, CPI 발표, 고용지표, 실적시즌 | ✅ 지표·섹터와 연결 |
| **섹터** | `sector` | 7~8개 MECE 산업군 | §5.2 | ✅ 매크로와 연결 |
...
### 5.2 섹터 분류 초안 (7~8개 MECE)

| 섹터 | 포함 |
| --- | --- |
| 반도체·IT하드웨어 | 삼성전자, SK하이닉스, 한미반도체 … |
| 인터넷·소프트웨어 | NAVER, 카카오, 크래프톤 … |
| 2차전지·화학소재 | LG에너지솔루션, 삼성SDI, LG화학 … |
| 운송장비 | 현대차, 기아, HD현대중공업, 한화에어로스페이스 … |
| 금융 | KB금융, 신한지주, 삼성생명 … |
| 헬스케어 | 삼성바이오로직스, 셀트리온 … |
| 소비재·유통·통신 | CJ제일제당, SK텔레콤, 하이브 … |
| 에너지·산업재 | POSCO홀딩스, S-Oil, 현대건설 … |

- **1종목 1섹터 원칙**. 지주사(삼성물산·SK·LG 등)만 예외 규칙 적용.
- 【미확정】 S8(에너지·소재·건설·운송)을 2개로 쪼개 9개 섹터로 갈지 — 유가 시그니처가 상반되는 종목이 섞임. 8/1 안건 1.

> **📄 상세 초안**: [섹터·종목매핑·관계DB 초안](./섹터-종목매핑-관계DB-초안.md) — 섹터 8개 정의 · **KOSPI 100 전체 매핑** · **관계DB 67행**(방향·부호·강도·근거·확신도) · 삭제 테스트로 제외한 관계 · 강도 실증 계획.
...
**규모**: 지표 8 × 섹터 8 = 상한 64행(유의미 ~40) + `sets` 8 + `transmits` 6 + `triggers` 6 ≒ **60~70행**. 멤버십 100행 별도.
...
| **관계DB** | 60~70행 큐레이션 (방향·부호·강도·근거) | `RelationDB` (분기) | P0 | (신규) |
...
- ⚠️ **동시 요청 함정**: 캐시는 첫 응답이 스트리밍을 시작해야 읽을 수 있다. **섹터 에이전트 7~8개를 한꺼번에 던지면 전원 캐시 미스**가 난다. → 1개를 먼저 보내고 첫 토큰 수신 후 나머지를 팬아웃한다.
...
- **In(MVP)**: KOSPI 1시장 · 섹터 7~8개 · 관계DB 60~70행 · 일간 BaseModel + 인사이트 리포트 · 이벤트 카드 3~5개 · 나 트윈 기본 · 대가 4인 참고 시뮬 · 편향 피드백.
